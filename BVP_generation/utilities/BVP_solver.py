...
def solve_newton(n, m, h, col_fun, bc, jac, y, p, B, bvp_tol, bc_tol, x):
    ...
    for iteration in range(max_iter):
        ...
        for trial in range(n_trial + 1):
            y_new = y - alpha * y_step  # y_step is not updated?
            # modify y_new
            # y_new = y_modify(m, y_new, x)  # should we put y outside the newton solver?
            if B is not None:
...
def solve_bvp(fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None,
    ...
    while True:
        ...
        sol = create_spline(y, f, x, h)  # interpolation after modifying the mesh for x
        ...
        if nodes_added > 0:
            x = modify_mesh(x, insert_1, insert_2)  # insert new node to modify the mesh
            h = np.diff(x)
            y = sol(x)
