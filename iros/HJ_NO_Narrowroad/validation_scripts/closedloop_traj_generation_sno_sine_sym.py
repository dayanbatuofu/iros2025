...
def value_action(X, t, model, alpha, param_fun):
    ...
    dx_1 = 2.0 * (X[0, :] - 15) / (95 - 15) - 1.
    dy_1 = 2.0 * (X[1, :] - 31) / (39 - 31) - 1.
    theta_1 = 2.0 * (X[2, :] - (-0.2)) / (0.2 - (-0.2)) - 1.
    v_1 = 2.0 * (X[3, :] - 18) / (29 - 18) - 1.
    dx_2 = 2.0 * (X[4, :] - 15) / (95 - 15) - 1.
    dy_2 = 2.0 * (X[5, :] - 31) / (39 - 31) - 1.
    theta_2 = 2.0 * (X[6, :] - (-0.2)) / (0.2 - (-0.2)) - 1.
    v_2 = 2.0 * (X[7, :] - 18) / (29 - 18) - 1.

    X = np.vstack((dx_1, dy_1, theta_1, v_1, dx_2, dy_2, theta_2, v_2))

    X = torch.tensor(X, dtype=torch.float32, requires_grad=True).T
    t = torch.tensor(t, dtype=torch.float32, requires_grad=True)
    coords_1 = torch.cat((t, X), dim=1)
    coords_2 = torch.cat((t, (torch.cat((X[:, 4:], X[:, :4]), dim=1))), dim=1)
...
def discrete_data(data, dt, theta, N):
    ...
    alpha = 1e-06
    beta = 10000
    threshold = 0.1 * (para_theta1 + para_theta2) + 0.05 * min(para_theta1, para_theta2) + 1.25
...
def pointInRect(points, threshold):
    for p in points:
        if np.sqrt((70 - p[2] - p[0]) ** 2 + (p[3] - p[1]) ** 2) <= threshold:
            return True
    return False
...
if __name__ == '__main__':
    ...
    model = modules_sno.SingleBVPNet(in_features=9, out_features=64, type=activation, mode='mlp',
    ...
    for n in range(len(idx0)):
        ...
        dist[n][0] = np.sqrt(((70 - dx2[n][0]) - dx1[n][0]) ** 2 + (dy2[n][0] - dy1[n][0]) ** 2)
    ...
    for i in range(X0.shape[1]):
        for j in range(1, Time.shape[0] + 1):
            ...
            if j == Time.shape[0]:
                ...
                dist[i][j] = np.sqrt(((70 - dx2[i][j]) - dx1[i][j]) ** 2 + (dy2[i][j] - dy1[i][j]) ** 2)
