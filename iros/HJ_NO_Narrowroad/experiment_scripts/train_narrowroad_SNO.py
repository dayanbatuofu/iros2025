...
200000 for hybrid neural operator
"""
...
supervised_dataset = dataio.NarrowHJI_Supervised(Hybrid_use, seed=opt.seed)
supervised_dataloader = DataLoader(supervised_dataset, shuffle=True, batch_size=opt.batch_size, pin_memory=True,
                                   num_workers=0)

model = modules_sno.SingleBVPNet(in_features=9, out_features=64, type=opt.model, mode=opt.mode,
                                 final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl)

model.to(device)

loss_fn_supervised = loss_functions.initialize_narrow_HJI_supervised(supervised_dataset, Weight1, alpha)
