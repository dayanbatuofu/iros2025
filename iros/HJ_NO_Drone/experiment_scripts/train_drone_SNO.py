...
model = modules_sno.SingleBVPNet(in_features=13, out_features=64, type=opt.model, mode=opt.mode,
                                 final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl)

model.to(device)

loss_fn_supervised = loss_functions.initialize_drone_HJI_supervised(supervised_dataset, Weight1, alpha)
