...
200000 for supervised neural operator
"""
...
200000 for SNO 
"""
p.add_argument('--pretrain_iters', type=int, default=200000, required=False, help='Number of pretrain iterations')
...
0 for SNO
"""
p.add_argument('--counter_end', type=int, default=0, required=False, help='Defines the linear step for curriculum training starting from the initial time')
...
Weight1 = (40, 500)
Weight2 = (40, 500, 6, 5000)
alpha = 0.1

Hybrid_use = False

supervised_dataset = dataio.LaneHJI_Supervised(Hybrid_use, seed=opt.seed)
supervised_dataloader = DataLoader(supervised_dataset, shuffle=True, batch_size=opt.batch_size, pin_memory=True,
                                   num_workers=0)

model = modules_sno.SingleBVPNet(in_features=10, out_features=64, type=opt.model, mode=opt.mode,
                                 final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl)

model.to(device)

loss_fn_supervised = loss_functions.initialize_lane_HJI_supervised(supervised_dataset, Weight1, alpha)

path = 'experiment_HJI_' + opt.model + '_sno/'
root_path = os.path.join(opt.logging_root, path)

training_sno.train(model=model, train_dataloader=supervised_dataloader,
                   ...
                   loss_fn=loss_fn_supervised, clip_grad=opt.clip_grad, use_lbfgs=opt.use_lbfgs,
