...
def value_action(X, t, model, alpha, param_fun):
    ...
    dy_1 = 2.0 * (X[1, :] - 33) / (39 - 33) - 1.
    theta_1 = 2.0 * (X[2, :] - (-0.15)) / (0.17 - (-0.15)) - 1.
    v_1 = 2.0 * (X[3, :] - 17) / (26 - 17) - 1.
    dx_2 = 2.0 * (X[4, :] - 0) / (95 - 0) - 1.
    dy_2 = 2.0 * (X[5, :] - 31) / (37 - 31) - 1.
    theta_2 = 2.0 * (X[6, :] - (-0.17)) / (0.15 - (-0.17)) - 1.
    v_2 = 2.0 * (X[7, :] - 17) / (26 - 17) - 1.
    label1 = torch.ones(1, 1)
    label2 = torch.zeros(1, 1)
    ...
    lam11_3 = dvdx_1[:, 2:3] / ((0.17 - (-0.15)) / 2)  # lambda_11
    ...
    lam22_3 = dvdx_2[:, 2:3] / ((0.15 - (-0.17)) / 2)  # lambda_22
...
def discrete_data(data, dt, theta, N):
    ...
    w2 = data['w2']
    time_horizon = N
    para_theta1, para_theta2 = theta

    alpha = 1e-06
    beta = 10000
    threshold = 0.1 * (para_theta1 + para_theta2) + 0.05 * min(para_theta1, para_theta2) + 2.25
...
def pointInRect(points, threshold):
    for p in points:
        if np.sqrt((p[2] - p[0]) ** 2 + (p[3] - p[1]) ** 2) <= threshold:
            return True
    return False
...
if __name__ == '__main__':
    ...
    policy1 = ['2_1', '3_1', '4_1', '5_1', '3_2', '4_2', '5_2', '4_3', '5_3', '5_4']
    param_type_P1 = ['theta_21', 'theta_31', 'theta_41', 'theta_51', 'theta_32', 'theta_42', 'theta_52', 'theta_43', 'theta_53', 'theta_54']
    param_type_P2 = ['theta_12', 'theta_13', 'theta_14', 'theta_15', 'theta_23', 'theta_24', 'theta_25', 'theta_34', 'theta_35', 'theta_45']
    ...
    dist = np.zeros((len(idx0), Time.shape[0]))
    ...
    for n in range(len(idx0)):
        ...
        dist[n][0] = np.sqrt(((dx2[n][0]) - dx1[n][0]) ** 2 + (dy2[n][0] - dy1[n][0]) ** 2)
    ...
    for i in range(X0.shape[1]):
        for j in range(1, Time.shape[0] + 1):
            ...
            u1[i][j - 1], u2[i][j - 1], w1[i][j - 1], w2[i][j - 1] = value_action(X_nn, t_nn, model, alpha, param_fun)
            if j == Time.shape[0]:
                break
            else:
                ...
                dist[i][j] = np.sqrt(((dx2[i][j]) - dx1[i][j]) ** 2 + (dy2[i][j] - dy1[i][j]) ** 2)

        pairs = zip(dx1[i, :], dy1[i, :], dx2[i, :], dy2[i, :])
        if pointInRect(pairs, threshold):
            count += 1
    ...
    if save_data:
        save_path = 'closed_loop/HNO/closedloop_traj_hno_initial_' + str(policy1[N_choice]) + '_relu.mat'
