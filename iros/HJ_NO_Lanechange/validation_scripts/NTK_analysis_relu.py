...
def loss_fn(model_output, threshold, alpha):
    ...
    lam11_1 = dvdx_1[:, :1] / ((95 - 0) / 2) * alpha  # lambda_11
    lam11_2 = dvdx_1[:, 1:2] / ((37 - 31) / 2) * alpha  # lambda_11
    lam11_3 = dvdx_1[:, 2:3] / ((0.15 - (-0.17)) / 2) * alpha  # lambda_11
    lam11_4 = dvdx_1[:, 3:4] / ((26 - 17) / 2) * alpha  # lambda_11
    lam12_1 = dvdx_1[:, 4:5] / ((95 - 0) / 2) * alpha  # lambda_12
    lam12_2 = dvdx_1[:, 5:6] / ((39 - 33) / 2) * alpha  # lambda_12
    lam12_3 = dvdx_1[:, 6:7] / ((0.17 - (-0.15)) / 2) * alpha  # lambda_12
    lam12_4 = dvdx_1[:, 7:8] / ((26 - 17) / 2) * alpha  # lambda_12
    ...
    lam21_1 = dvdx_2[:, 4:5] / ((95 - 0) / 2) * alpha  # lambda_21
    lam21_2 = dvdx_2[:, 5:6] / ((37 - 31) / 2) * alpha  # lambda_21
    lam21_3 = dvdx_2[:, 6:7] / ((0.15 - (-0.17)) / 2) * alpha  # lambda_21
    lam21_4 = dvdx_2[:, 7:8] / ((26 - 17) / 2) * alpha  # lambda_21
    lam22_1 = dvdx_2[:, :1] / ((95 - 0) / 2) * alpha  # lambda_22
    lam22_2 = dvdx_2[:, 1:2] / ((39 - 33) / 2) * alpha  # lambda_22
    lam22_3 = dvdx_2[:, 2:3] / ((0.17 - (-0.15)) / 2) * alpha  # lambda_22
    lam22_4 = dvdx_2[:, 3:4] / ((26 - 17) / 2) * alpha  # lambda_22
    ...
    dx_11_sl = (x1[:, :num_sl, 1:2] + 1) * (95 - 0) / 2 + 0
    dy_11_sl = (x1[:, :num_sl, 2:3] + 1) * (37 - 31) / 2 + 31

    dx_11_hl = (x1[:, num_sl:, 1:2] + 1) * (95 - 0) / 2 + 0
    dy_11_hl = (x1[:, num_sl:, 2:3] + 1) * (37 - 31) / 2 + 31

    theta_11 = (x1[:, :, 3:4] + 1) * (0.15 - (-0.17)) / 2 + (-0.17)
    v_11 = (x1[:, :, 4:5] + 1) * (26 - 17) / 2 + 17

    # unnormalize the state for agent 2
    dx_12_sl = (x1[:, :num_sl, 5:6] + 1) * (95 - 0) / 2 + 0
    dy_12_sl = (x1[:, :num_sl, 6:7] + 1) * (39 - 33) / 2 + 33

    dx_12_hl = (x1[:, num_sl:, 5:6] + 1) * (95 - 0) / 2 + 0
    dy_12_hl = (x1[:, num_sl:, 6:7] + 1) * (39 - 33) / 2 + 33

    theta_12 = (x1[:, :, 7:8] + 1) * (0.17 - (-0.15)) / 2 + (-0.15)
    v_12 = (x1[:, :, 8:9] + 1) * (26 - 17) / 2 + 17

    # calculate the collision area lower and upper bounds
    dist_diff1_sl = (-(torch.sqrt((dx_12_sl - dx_11_sl) ** 2 + (dy_12_sl - dy_11_sl) ** 2)
                     - threshold) * 5).squeeze().reshape(-1, 1).to(device)
    dist_diff1_hl = (-(torch.sqrt((dx_12_hl - dx_11_hl) ** 2 + (dy_12_hl - dy_11_hl) ** 2)
                     - threshold) * 5).squeeze().reshape(-1, 1).to(device)
    ...
    # unnormalize the state for agent 1
    dx_21_sl = (x2[:, :num_sl, 5:6] + 1) * (95 - 0) / 2 + 0
    dy_21_sl = (x2[:, :num_sl, 6:7] + 1) * (37 - 31) / 2 + 31

    dx_21_hl = (x2[:, num_sl:, 5:6] + 1) * (95 - 0) / 2 + 0
    dy_21_hl = (x2[:, num_sl:, 6:7] + 1) * (37 - 31) / 2 + 31

    theta_21 = (x2[:, :, 7:8] + 1) * (0.15 - (-0.17)) / 2 + (-0.17)
    v_21 = (x2[:, :, 8:9] + 1) * (26 - 17) / 2 + 17

    # unnormalize the state for agent 2
    dx_22_sl = (x2[:, :num_sl, 1:2] + 1) * (95 - 0) / 2 + 0
    dy_22_sl = (x2[:, :num_sl, 2:3] + 1) * (39 - 33) / 2 + 33

    dx_22_hl = (x2[:, num_sl:, 1:2] + 1) * (95 - 0) / 2 + 0
    dy_22_hl = (x2[:, num_sl:, 2:3] + 1) * (39 - 33) / 2 + 33

    theta_22 = (x2[:, :, 3:4] + 1) * (0.17 - (-0.15)) / 2 + (-0.15)
    v_22 = (x2[:, :, 4:5] + 1) * (26 - 17) / 2 + 17

    # calculate the collision area lower and upper bounds
    dist_diff2_sl = (-(torch.sqrt((dx_22_sl - dx_21_sl) ** 2 + (dy_22_sl - dy_21_sl) ** 2)
                     - threshold) * 5).squeeze().reshape(-1, 1).to(device)
    dist_diff2_hl = (-(torch.sqrt((dx_22_hl - dx_21_hl) ** 2 + (dy_22_hl - dy_21_hl) ** 2)
                     - threshold) * 5).squeeze().reshape(-1, 1).to(device)
    ...
    y_c = torch.cat((alpha*costate1_prediction,
                     alpha*costate2_prediction), dim=0)

    return {'v_pre_rr': diff_constraint_hom / 5000,
...
if __name__ == '__main__':
    ...
    for n_sample in range(2):
        ...
        for i in range(4):
            ...
            threshold = 0.1 * (theta1 + theta2) + 0.05 * min(theta1, theta2) + 2.25
            count = 0

            print(N_choice, threshold)

            ckpt_path = './model/relu/model_hno_relu.pth'
            activation = 'relu'

            # Initialize and load the model
            model = modules_hno.SingleBVPNet(in_features=10, out_features=64, type=activation, mode='mlp',
            ...
            data_path = './train_data/lanechange_param_fun_40.mat'
            ...
            path = './train_data/data_train_lanechange_' + str(policy[N_choice]) + '_1000.mat'
            # path = './test_data/data_test_lanechange_' + str(policy[N_choice]) + '_600.mat'
            ...
            lb = torch.tensor([[0], [31], [-0.17], [17], [0], [33], [-0.15], [17]], dtype=torch.float32)
            ub = torch.tensor([[95], [37], [0.15], [26], [95], [39], [0.17], [26]], dtype=torch.float32)
            ...
            label1 = torch.zeros(coords_1.shape[0], 1)
            label2 = torch.ones(coords_2.shape[0], 1)
        ...
        log_dir = 'NTK/'
        cond_num_file = 'condition_num_relu' + str(n_sample) + '.txt'
        erank_file = 'erank_relu' + str(n_sample) + '.txt'
