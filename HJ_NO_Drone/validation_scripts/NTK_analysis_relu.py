...
if __name__ == '__main__':
    ...
    for n_sample in range(2):
        ...
        for i in range(4):
            ...
            # coords1_pinn = torch.zeros(N_sample, 12).uniform_(-1, 1)
            # coords2_pinn = torch.cat((coords1_pinn[:, 6:], coords1_pinn[:, :6]), dim=1)
            #
            # # slowly grow time values from start time
            # # this currently assumes start_time = 0 and max time value is tMax
            # time = torch.zeros(N_sample, 1).uniform_(0, 4)
            #
            # coords1_pinn = torch.cat((time, coords1_pinn), dim=1)
            # coords2_pinn = torch.cat((time, coords2_pinn), dim=1)
            #
            # coords_pinn = torch.cat((coords1_pinn, coords2_pinn), dim=0)
            #
            # torch.save(coords_pinn, "data_sample.pt")
