...
def discrete_data(data, dt, theta, N):
    ...
    alpha = 1e-06
    beta = 10000
    threshold = 0.1*(para_theta1+para_theta2) + 0.05*min(para_theta1, para_theta2) + 0.5
    gravity = 9.81
...
def pointInRect(points, threshold):
    for p in points:
        if np.sqrt((5 - p[3] - p[0]) ** 2 + ((5 - p[4]) - p[1]) ** 2 + (p[5] - p[2]) ** 2) <= threshold:
            return True
    return False
...
if __name__ == '__main__':
    ...
    for n in range(len(idx0)):
        ...
        dist[n][0] = np.sqrt(((5 - dx2[n][0]) - dx1[n][0]) ** 2 + ((5 - dy2[n][0]) - dy1[n][0]) ** 2 + (dz2[n][0] - dz1[n][0]) ** 2)
    ...
    for i in range(X0.shape[1]):
        for j in range(1, Time.shape[0] + 1):
            ...
            thrust1[i][j - 1], thrust2[i][j - 1] = value_action(X_nn, t_nn, model, alpha, param_fun)
            if j == Time.shape[0]:
                break
            else:
                ...
                dist[i][j] = np.sqrt(((5 - dx2[i][j]) - dx1[i][j]) ** 2 + ((5 - dy2[i][j]) - dy1[i][j]) ** 2 + (dz2[i][j] - dz1[i][j]) ** 2)

        pairs = zip(dx1[i, :], dy1[i, :], dz1[i, :], dx2[i, :], dy2[i, :], dz2[i, :])
